...
export class GamesController {
    ...
    async createGame(@UploadedFile() file: Express.Multer.File, @Body() createGameDto: CreateGameDto) {
        const imageUrl: string = uploadImage(createGameDto.image);
        const gameSession = this.gameSessionService.createGameSession(createGameDto, imageUrl);
        return gameSession;
    }
}